...
```{r}
png(filename="/Users/lennonj/GitHub/SporeMut/output/1.PopDynamics/1.PopDynamics_Log_VST.png",
...
plot(log10(data2$time), log10(data2$v.est*10^4), pch = 21, 
     ...
     xlab = NA, ylab = NA, xlim = c(-0.5, 3.2), ylim = c(4, 10), yaxt = "n", xaxt = "n")

points(log10(data2$time), log10(data2$s.est*10^4), pch = 21, lwd = 0.75, col = "grey50", 
      bg = "grey50", cex = 1.25)


data.all <- data.frame(data.all, data.all$spore + data.all$veg)
colnames(data.all) <- c("time", "spore", "veg", "total")

#data2.T <- data.frame(data2$time, (data2$s.est*10^4 + data2$v.est*10^4))
#colnames(data2.T) <-c("time", "total")

#points(log10(data.all$time), log10(data.all$total), pch = 21, lwd = 0.75, col = "black", 
#      bg = "black", cex = 1.25)

legend(50, 9, c("Spore", "Vegetative", "Total"), pch = 21, pt.bg = c("grey50", "grey80", "black"), 
       col = c("grey50", "grey80", "black"), pt.cex = 1.25, pt.lwd = 0.75, bty = 'n', 
...
curve(coef(fit2)[[2]] + (coef(fit2)[[1]] - coef(fit2)[[2]]) / 
        (1 + exp((coef(fit2)[[3]] - x) / coef(fit2)[[4]])), 
      from = 0, to = 3, add = TRUE, lty = 2, lwd = 2.5, col = "grey80") 
...
curve(coef(fit3)[[2]] + (coef(fit3)[[1]] - coef(fit3)[[2]]) / 
      ...
      from = 0, to = 3, add = TRUE, lty = 2, lwd = 2.5, col = "grey50") 
...
data.all <- data.frame(data.all, data.all$spore + data.all$veg)
colnames(data.all) <- c("time", "spore", "veg", "total")
fit4 <- mle2(log10(total) ~ dnorm(mean = b + (a - b) / (1 + exp((m - log10(time)) / w)), sd = z),
            start = list(a = A, b = B, m = M, w = W, z = Z), data = data.all) 

curve(coef(fit4)[[2]] + (coef(fit4)[[1]] - coef(fit4)[[2]]) / 
        (1 + exp((coef(fit4)[[3]] - x) / coef(fit4)[[4]])), 
...
# Show Plot
img <- readPNG("/Users/lennonj/GitHub/SporeMut/output/1.PopDynamics/1.PopDynamics_Log_VST.png")
grid.raster(img)
```